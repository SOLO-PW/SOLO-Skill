...
description: |
  就业方向分析与发展路径规划。当用户提供教育背景、技能证书、实习经验、个人特质等信息，或询问职业发展建议、就业方向、行业前景、薪资水平、职业规划时使用。支持主动提问补全需求，生成职业发展路径图。
---
...
| 场景 | 检索关键词示例 | 检索目的 |
|------|---------------|----------|
| 职位搜索 | "[职位名称] 招聘 要求 薪资 [城市] [当前年份]" | 了解岗位需求和薪资 |
| 行业分析 | "[行业名称] 发展趋势 市场规模 [当前年份]" | 了解行业前景 |
| 薪资查询 | "[职位名称] 平均薪资 [城市] [当前年份]" | 获取薪资数据 |
| 政策查询 | "[行业/地区] 就业政策 人才引进" | 了解政策支持 |
| 企业分析 | "[企业类型] 企业文化 工作环境" | 了解企业特点 |

> **动态年份说明**：检索时应使用实际当前年份替换 `[当前年份]` 占位符（即检索发生时的年份），避免使用过期年份导致信息过时。
...
3. **权威性**：官方数据优先于民间数据
4. **代表性**：关注数据样本量和覆盖范围
5. **数据新鲜度标注**：报告中的每个薪资、行业、市场规模数据点，均需标注**数据来源**、**检索/更新时间（至少到年份）**、**数据口径/样本量**，以便最终报告可追溯。
...
**权重校验提示**：以上各维度权重之和应为 1.0（0.2 + 0.3 + 0.25 + 0.15 + 0.1 = 1.0）。计算前请确认权重未遗漏或重复，保证加权结果落在 0-100 区间。

**数值算例**（给定各维度得分 → 加权求和 → 映射匹配等级）：
...
加权求和：`90×0.2 + 80×0.3 + 70×0.25 + 80×0.15 + 80×0.1 = 18 + 24 + 17.5 + 12 + 8 = 79.5`，对应匹配等级「一般匹配」。更详细的完整算例可参考 [algorithms.md 示例1](references/algorithms.md)（匹配度 80.3 → 较好匹配）。
...
### 薪资水平
[相关职位薪资范围]
- 数据来源/检索时间/口径（示例：职友集，2025-06，N=1,200 份样本）
...
### 行业分析
[行业发展趋势和前景]
- 数据来源/检索时间/口径（示例：艾瑞咨询报告，2025-08）

### 薪资分析
[薪资水平和增长趋势]
- 数据来源/检索时间/口径（示例：国家统计局，2025年度平均工资）
